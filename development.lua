...
-- Development keymaps
vim.keymap.set('n', '<leader>nr', ':lua require("nimure").refresh_resources()<CR>', { desc = "Refresh Nimure resources" })
vim.keymap.set('n', '<leader>nt', ':lua require("nimure").toggle_sidebar()<CR>', { desc = "Toggle Nimure sidebar" })
vim.keymap.set('n', '<leader>nf', ':lua require("nimure").toggle_floating_right()<CR>', { desc = "Toggle floating sidebar" })
...
vim.schedule(function()
  ...
  vim.notify("Available commands: :NimureToggle, :NimureFloatRight, :NimureRefresh, :NimureSearch, :NimureHealth, :NimureReload", vim.log.levels.INFO)
  vim.notify("Keymaps: <leader>nt (toggle), <leader>nf (float right), <leader>nr (refresh), <leader>ns (search), <leader>nh (health)", vim.log.levels.INFO)
end) 