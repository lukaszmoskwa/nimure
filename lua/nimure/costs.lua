...
function M.generate_daily_chart(daily_costs, height)
  ...
  for i, day in ipairs(daily_costs) do
    ...
    table.insert(chart_lines, string.format(
      "%s │%s %.2f", 
      date_short, 
      bar .. string.rep(" ", 50 - bar_length), 
      day.cost
    ))
...
function M.generate_service_chart(services, max_services)
  ...
  -- Create chart header
  table.insert(chart_lines, string.format("Service Breakdown (Total: %.2f %s)", 
    total_cost, services[1].currency))
...
function M.format_cost_summary(cost_data)
  ...
  table.insert(summary_lines, string.format("💰 Total Cost: %s%.2f %s", 
    currency_symbol, cost_data.total_cost, cost_data.currency))
  table.insert(summary_lines, string.format("📅 Period: %s to %s", 
    cost_data.period.start_date, cost_data.period.end_date))
  ...
  if #cost_data.daily_costs > 0 then
    ...
    table.insert(summary_lines, string.format("📈 Daily Average: %s%.2f %s", 
      currency_symbol, avg_daily, cost_data.currency))
  end
  ...
  if #cost_data.services > 0 then
    ...
    for i = 1, math.min(3, #cost_data.services) do
      ...
      table.insert(summary_lines, string.format("   %d. %s: %s%.2f (%.1f%%)", 
        i, service.name, currency_symbol, service.cost, percentage))
    end
  end
...
function M.format_resource_costs(resource_costs)
  ...
  table.insert(lines, string.format("Total Cost: %s%.2f %s", 
    currency_symbol, resource_costs.total_cost, resource_costs.currency))
