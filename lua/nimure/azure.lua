	...
	if account_info.user and account_info.user.name then
		...
		if email_domain then
			...
			if email_domain:find("%.uk$") or email_domain:find("%.gb$") then
				currency = "GBP"
			elseif
				email_domain:find("%.de$")
				or email_domain:find("%.fr$")
				or email_domain:find("%.it$")
				or email_domain:find("%.es$")
				or email_domain:find("%.nl$")
				or email_domain:find("%.at$")
			then
				currency = "EUR"
...
function M.get_subscription_costs(options, callback)
	...
	if
		...
		and M.cache.cost_data[cache_key].timestamp
		and is_cache_valid(M.cache.cost_data[cache_key].timestamp)
	then
		vim.schedule(function()
...
function M.process_cost_management_data(response, start_date, end_date)
	...
	if response.properties then
		...
		if
			...
			and type(response.properties.currency) == "string"
			and response.properties.currency ~= vim.NIL
		then
			currency = response.properties.currency
		elseif
			response.properties.nextLink
			and type(response.properties.nextLink) == "string"
			and response.properties.nextLink ~= vim.NIL
		then
	...
	for _, row in ipairs(response.properties.rows) do
		...
		if
			currency == "USD"
			and row_currency
			and type(row_currency) == "string"
			and row_currency ~= vim.NIL
			and #row_currency > 0
		then
		...
		Job:new({
			...
			on_exit = function(_, return_val)
				...
				callback({
					...
					note = string.format(
						"Showing costs for resource group '%s' (period: %s to %s)",
